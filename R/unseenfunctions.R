  ...
  for(j in 1:p) {			
    if(complete.family[j] != "multinom") {
      if(length(unique(complete.family)) == 1) {
        if(j == 1) {
          if(num.lv == 0){ 
            ...
            linpred_string2 <- paste("etaStar[i,j] <- 0", sep = "")
          }
          if(num.lv > 0) {
            linpred_string <- paste("eta[i,j] <- inprod(lv.coefs[j,2:(num.lv+1)],lvs[i,])", sep = "")
            linpred_string2 <-paste("etaStar[i,j] <- inprod(lv.coefs[j,2:(num.lv+1)],lvsStar[i,])", sep = "")
          ...
          if(num.X > 0){ 
            ...
            linpred_string2 <- paste(linpred_string2, " + inprod(X.coefs[j,],X[i,])", sep ="")
          }
          if(!is.null(offset)){ 
            linpred_string <- paste(linpred_string, " + offset[i,j]", sep ="")
            linpred_string2 <- paste(linpred_string2, " + offset[i,j]", sep ="")
          } 
          respfamily_script <- c(respfamily_script, paste("\t\t for(j in 1:p) { ", linpred_string, " }", sep = ""))
          respfamily_script <- c(respfamily_script, paste("\t\t for(j in 1:p) { ", linpred_string2, " }", sep = ""))
      ...
      if(length(unique(complete.family)) > 1) {		
        ...
        if(num.lv > 0){
          linpred_string <- paste("eta[i,",j, "] <- inprod(lv.coefs[",j, ",2:(num.lv+1)],lvs[i,])", sep = "")
          linpred_string2 <- paste("etaStar[i,",j, "] <- inprod(lv.coefs[",j, ",2:(num.lv+1)],lvs[i,])", sep = "")
        ...
        respfamily_script <- c(respfamily_script, paste("\t\t ", linpred_string2, sep = ""))
    ...
    if(all(complete.family == "bernoulli")) { ## If all data are Bernoulli, then use step parameterization to speed up sampling!
      if(length(unique(complete.family)) == 1) {
        if(j == 1) {
          ...
          respfamily_script <- c(respfamily_script, paste("\t\t for(j in 1:p) { ZStar[i,j] ~ dnorm(lv.coefs[j,1] + etaStar[i,j], 1) }", sep = ""))
          
          respfamily_script <- c(respfamily_script, paste("\t\t for(j in 1:p) { y[i,j] ~ dbern(step(Z[i,j])) }\n", sep = ""))
          respfamily_script <- c(respfamily_script, paste("\t\t for(j in 1:p) { yStar[i,j] ~ dbern(step(ZStar[i,j])) }\n", sep = ""))
      ...
      if(length(unique(complete.family)) > 1) {		
        ...
        respfamily_script <- c(respfamily_script, paste("\t\t ZStar[i,",j, "] ~ dnorm(lv.coefs[", j, ",1] + etaStar[i,",j, "], 1)", sep = ""))
        
        respfamily_script <- c(respfamily_script, paste("\t\t y[i,",j, "] ~ dbern(step(Z[i,",j, "]))\n", sep = ""))
        respfamily_script <- c(respfamily_script, paste("\t\t yStar[i,",j, "] ~ dbern(step(ZStar[i,",j, "]))\n", sep = ""))
